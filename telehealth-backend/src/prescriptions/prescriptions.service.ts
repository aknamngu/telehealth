...
export class PrescriptionsService {
  ...
  async create(createPrescriptionDto: CreatePrescriptionDto, user: { sub: number; role: string }) {
    ...
    if (user.role === 'DOCTOR' && appointment.doctorId !== user.sub) {
      throw new ForbiddenException('Bác sĩ chỉ có thể kê đơn cho lịch hẹn của chính mình!');
    }
