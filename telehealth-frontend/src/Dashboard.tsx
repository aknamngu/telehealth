...
function Dashboard() {
  ...
  const [payload, setPayload] = useState<
    AdminPayload | PatientPayload | DoctorPayload | null
  >(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [cancellingId, setCancellingId] = useState<number | null>(null);
  const [newApptNotif, setNewApptNotif] = useState<NewAppointmentNotif | null>(
    null,
  );
  ...
  const [allPrescriptions, setAllPrescriptions] = useState<
    { id: number; diagnosis: string; medicines: string; doctorName?: string }[]
  >([]);
  ...
  const loadDashboard = useCallback(() => {
    ...
    fetch(`${API_URL}/dashboard`, {
      ...
      .then(async (response) => {
        const data = (await response.json()) as ApiWrapper<
          AdminPayload | PatientPayload | DoctorPayload
        >;
        if (!response.ok)
          throw new Error(
            data.message ??
              tr("Không thể tải bảng điều khiển", "Unable to load dashboard"),
          );
  ...
  const loadWalletAndInvoices = useCallback(() => {
    ...
    if (authUser.role === "PATIENT") {
      fetch(`${API_URL}/wallet/invoices/me`, {
        headers: { Authorization: `Bearer ${token}` },
      })
        .then((res) => res.json())
        .then((data) => {
  ...
  const loadDoctorSchedules = useCallback(() => {
    if (!authUser || authUser.role !== "DOCTOR" || !selectedScheduleDate)
      return;
    const token = getAuthToken();
    setLoadingSchedule(true);
    fetch(
      `${API_URL}/doctors/${authUser.id}/schedules?date=${selectedScheduleDate}`,
      {
        headers: { Authorization: `Bearer ${token}` },
      },
    )
  ...
  const roleLabel = useMemo(() => {
    if (authUser?.role === "ADMIN")
      return tr("Tổng quan quản trị", "Admin Overview");
    if (authUser?.role === "PATIENT")
      return tr("Trung tâm chăm sóc bệnh nhân", "Patient Care Center");
    if (authUser?.role === "DOCTOR")
      return tr("Trung tâm điều hành bác sĩ", "Doctor Command Room");
  ...
  const localizeSpecialty = useCallback(
    (specialty?: string) => {
        ...
        (
          {
            ...
            "Chuyên khoa Tim mạch & Cấp cứu AI":
              "Cardiology & AI Emergency Care",
            "Chuyên khoa Nội tổng quát & Chăm sóc từ xa":
              "General Medicine & Remote Care",
  ...
  const statCards = useMemo(() => {
    ...
    return [
      ...
      [
        tr("Sắp tới", "Upcoming"),
        doctor.stats.upcomingAppointments ?? doctor.stats.todayAppointments,
      ],
  ...
  const handleRefund = async (invoiceId: number) => {
    showConfirm({
      ...
      onConfirm: async () => {
        ...
        try {
          const res = await fetch(
            `${API_URL}/wallet/invoices/${invoiceId}/refund`,
            {
              method: "POST",
              headers: { Authorization: `Bearer ${token}` },
            },
          ...
          if (!res.ok)
            throw new Error(
              data.message || tr("Lỗi hoàn tiền", "Refund failed"),
            );
          showToast(
            "success",
            tr("✅ Hoàn tiền thành công!", "✅ Refund completed!"),
          );
          loadWalletAndInvoices();
        } catch (err: any) {
        ...
        try {
          ...
          if (res.ok) {
            showToast(
              "success",
              currentStatus
                ? tr("Đã khóa tài khoản!", "Account locked!")
                : tr("Đã mở khóa tài khoản!", "Account unlocked!"),
            );
            loadDashboard();
          } else {
            const data = await res.json();
            showToast(
              "error",
              data.message || tr("Có lỗi xảy ra", "An error occurred"),
            );
          }
        } catch (_e) {
          showToast(
  ...
  async function submitRating(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(
        `${API_URL}/appointments/${ratingModal.appointmentId}/review`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!res.ok)
        throw new Error(
          data.message || tr("Lỗi khi gửi đánh giá", "Unable to submit review"),
        );

      showToast(
        "success",
        tr("✨ Cảm ơn bạn đã gửi đánh giá!", "✨ Thank you for your review!"),
      );
      setRatingModal(null);
      setRatingVal(5);
      setRatingComment("");
      ...
      showToast(
        ...
        err instanceof Error
          ? err.message
          : tr("Có lỗi xảy ra", "An error occurred"),
      );
    } finally {
  ...
  const savePatientProfile = async () => {
    ...
    try {
      ...
      showToast(
        "success",
        tr("✅ Cập nhật hồ sơ thành công!", "✅ Profile updated successfully!"),
      );
    } catch (err) {
      showToast(
        "error",
        err instanceof Error
          ? err.message
          : tr("Lỗi lưu hồ sơ", "Unable to save profile"),
      );
    } finally {
    ...
    try {
      ...
      if (data.data?.doctorProfile) {
        ...
        setDoctorProfileForm({
          ...
          experienceYears: dp.experienceYears ?? 0,
          bio: dp.bio ?? "",
        });
      }
    } catch (e) {
  ...
  const saveDoctorProfile = async () => {
    ...
    try {
      ...
      showToast(
        "info",
        tr(
          "📨 Hồ sơ đã gửi, đang chờ quản trị viên phê duyệt!",
          "📨 Profile submitted for administrator approval!",
        ),
      );
    } catch (err) {
      showToast(
  ...
  const loadPendingDoctors = useCallback(async () => {
    ...
    const token = getAuthToken();
    try {
      const res = await fetch(`${API_URL}/users/doctors/pending`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
  ...
  const approveDoctorProfile = async (
    userId: number,
    ...
    try {
      const res = await fetch(
        `${API_URL}/users/${userId}/doctor-profile/approve`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
      ...
      showToast(
        ...
        status === "APPROVED"
          ? tr("✅ Đã phê duyệt hồ sơ!", "✅ Profile approved!")
          : tr("❌ Đã từ chối hồ sơ!", "❌ Profile rejected!"),
      );
      loadPendingDoctors();
    } catch (err) {
      showToast(
        ...
        err instanceof Error
          ? err.message
          : tr("Lỗi khi xử lý", "Unable to process request"),
      );
    }
  };
  ...
  const addReminder = async () => {
    if (!reminderForm.medicineName.trim() || !reminderForm.prescriptionId) {
      showToast(
        "warning",
        tr(
          "Vui lòng chọn đơn thuốc và điền tên thuốc!",
          "Select a prescription and enter the medication name!",
        ),
      );
      return;
    }
    ...
    try {
      ...
      showToast(
        "success",
        tr("💊 Đã thêm lịch nhắc uống thuốc!", "💊 Medication reminder added!"),
      );
    } catch (err) {
      showToast(
        "error",
        err instanceof Error
          ? err.message
          : tr("Lỗi thêm nhắc nhở", "Unable to add reminder"),
      );
    } finally {
  ...
  const deleteReminder = async (id: number) => {
    showConfirm({
      ...
      message: tr(
        "Bạn có chắc muốn xóa lịch nhắc uống thuốc này?",
  ...
  useEffect(() => {
    ...
    reminderCheckRef.current = setInterval(() => {
      ...
      const matched = reminders.find(
        (r) => r.isActive && r.reminderTime === hhmm,
      );
      if (matched) {
        setReminderToast(
          `💊 ${tr("Đến giờ uống thuốc", "Time to take your medication")}: ${matched.medicineName}`,
        );
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(14,165,233,0.12),_transparent_30%),radial-gradient(circle_at_top_right,_rgba(16,185,129,0.12),_transparent_26%),linear-gradient(180deg,_#f8fafc_0%,_#eef6ff_52%,_#f8fafc_100%)] text-slate-900">
          ...
          {toasts.map((toast) => {
            ...
            return (
              ...
              >
                ...
                <p className={`flex-1 text-sm font-semibold ${cfg.accent}`}>
                  {toast.message}
                </p>
                <button
        ...
        <div className="fixed inset-0 z-[250] flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-[2rem] bg-white p-8 shadow-2xl animate-[slideUp_0.3s_cubic-bezier(0.16,1,0.3,1)]">
            <div className="flex items-start gap-4">
              <div
                className={`grid h-12 w-12 shrink-0 place-items-center rounded-2xl ${confirmModal.variant === "danger" ? "bg-rose-100" : "bg-emerald-100"}`}
              >
        ...
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[200] flex items-start gap-3 rounded-2xl border border-violet-200 bg-white p-4 shadow-2xl shadow-violet-500/20 w-96">
          ...
          <div className="flex-1">
            <p className="text-xs font-bold uppercase tracking-wider text-violet-600">
              {tr("Nhắc uống thuốc", "Medication reminder")}
            </p>
        ...
        <div className="fixed bottom-6 right-6 z-50 w-80 overflow-hidden rounded-2xl border border-emerald-200 bg-white shadow-2xl shadow-emerald-500/20 animate-in slide-in-from-right">
          <div className="bg-gradient-to-r from-emerald-600 to-teal-500 px-4 py-3 text-white flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-black uppercase tracking-[0.2em]">
                {tr("Lịch hẹn mới!", "New appointment!")}
              </span>
            </div>
            <button
              onClick={() => setNewApptNotif(null)}
              className="grid h-6 w-6 place-items-center rounded-full bg-white/20 hover:bg-white/30"
            >
              <X className="h-3 w-3" />
            </button>
          </div>
          <div className="p-4">
            <p className="font-bold text-slate-900">
              {tr("Bệnh nhân", "Patient")} {newApptNotif.patientName}
            </p>
            <p className="mt-1 text-sm text-slate-600">
              {tr("Đặt lịch ngày", "Appointment on")}{" "}
              <strong>
                {new Date(newApptNotif.date).toLocaleDateString(
                  language === "vi" ? "vi-VN" : "en-US",
                )}
              </strong>{" "}
              {tr("lúc", "at")} <strong>{newApptNotif.startTime}</strong>
            </p>
            <div className="mt-3 flex gap-2">
              <button
                onClick={() => {
                  setNewApptNotif(null);
                  loadDashboard();
                }}
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 transition hover:border-sky-200 hover:text-sky-700 sm:px-4"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 transition hover:border-rose-200 hover:text-rose-700 sm:px-4"
            >
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl space-y-3">
              ...
              <h1 className="text-3xl font-black tracking-tight sm:text-4xl">
                {tr(
                  "Bảng điều khiển TeleHealth theo vai trò",
                  "Role-based TeleHealth dashboard",
                )}
              </h1>
              <p className="text-slate-600">
                {tr("Xin chào", "Hello")}{" "}
                {authUser?.fullName ?? tr("bạn", "there")} —{" "}
                {tr(
                  "dữ liệu được bảo vệ theo tài khoản đăng nhập hiện tại.",
                  "data is protected according to the currently signed-in account.",
                )}
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 rounded-3xl border border-slate-200 bg-slate-50 px-4 py-3">
              ...
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.2em] text-slate-500">
                  {tr("Vai trò hiện tại", "Current role")}
                </p>
        ...
        {loading ? (
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-2">
            {authUser.role === "ADMIN"
              ? renderAdminSections(payload as AdminPayload)
              : null}
            {authUser.role === "PATIENT"
              ? renderPatientSections(payload as PatientPayload)
              : null}
            {authUser.role === "DOCTOR"
              ? renderDoctorSections(payload as DoctorPayload)
              : null}
          </div>
        ) : null}
      </main>
      ...
      {selectedPrescription && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm">
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto animate-[slideUp_0.3s_cubic-bezier(0.16,1,0.3,1)] rounded-[2rem] bg-white p-8 shadow-2xl">
            <div className="mb-6 flex items-start justify-between">
              <div>
                <h3 className="text-2xl font-black tracking-tight text-slate-900">
                  {tr("Hồ sơ & đơn thuốc", "Medical record & prescription")}
                </h3>
                <p className="text-sm font-medium text-slate-500">
                  {tr("Khám ngày", "Consultation on")}{" "}
                  {selectedPrescription.date} {tr("với", "with")}{" "}
            ...
            <div className="space-y-6">
              ...
              {selectedPrescription.aiSummary && (
                <div className="mt-4">
                  <label className="mb-2 block text-xs font-bold uppercase tracking-[0.18em] text-sky-600">
                    ...
                    {tr(
                      "Tóm tắt bệnh án (bởi AI)",
                      "Medical record summary (AI)",
                    )}
                  </label>
              ...
              {selectedPrescription.verificationToken && (
                <div className="rounded-3xl border border-sky-100 bg-sky-50/60 p-5">
                  <div className="grid items-center gap-5 sm:grid-cols-[150px_1fr]">
                    <img
                      src={`${API_URL}/prescriptions/verify/${encodeURIComponent(selectedPrescription.verificationToken)}/qr?baseUrl=${encodeURIComponent(window.location.origin)}`}
                      alt={tr(
                        "QR xác minh đơn thuốc",
                        "Prescription verification QR",
                      )}
                      className="mx-auto h-[150px] w-[150px] rounded-2xl border border-white bg-white p-2 shadow-sm"
                    />
                    <div>
                      ...
                      <p className="mt-2 text-xs leading-5 text-slate-600">
                        {tr(
                          "Quét để kiểm tra bác sĩ, bệnh nhân, nội dung đơn và chữ ký kỹ thuật SHA-256.",
                          "Scan to verify the doctor, patient, prescription content and SHA-256 technical signature.",
                        )}
                      </p>
                      <a
                        href={`/prescriptions/verify/${encodeURIComponent(selectedPrescription.verificationToken)}`}
                        target="_blank"
                        rel="noreferrer"
                        className="mt-3 inline-flex rounded-full bg-sky-600 px-4 py-2 text-xs font-bold text-white hover:bg-sky-700"
                      >
                        {tr("Mở trang xác minh", "Open verification page")}
                      </a>
                    </div>
                  </div>
                </div>
            ...
            <div className="mb-4 flex items-start justify-between">
              <div>
                ...
                <p className="text-xs font-medium text-slate-500">
                  {tr("Bác sĩ", "Doctor")} {ratingModal.doctorName}
                </p>
              </div>
              <button
            ...
            <form onSubmit={submitRating} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder={tr(
                    "Bác sĩ tư vấn nhiệt tình...",
                    "The doctor was attentive and helpful...",
                  )}
      ...
      {showProfileModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-[2rem] bg-white p-8 shadow-2xl animate-in fade-in zoom-in-95">
            <div className="mb-6 flex items-start justify-between">
              <h3 className="text-2xl font-black tracking-tight text-slate-900">
                {tr("Hồ sơ y tế của tôi", "My medical profile")}
              </h3>
              <button
                onClick={() => setShowProfileModal(false)}
                className="grid h-8 w-8 place-items-center rounded-full bg-slate-100 hover:bg-slate-200"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder={tr("Ví dụ: O+, A-", "Example: O+, A-")}
                />
              </div>
              <div>
                <label className="mb-1 block text-sm font-bold text-slate-700">
                  {tr("Tiền sử bệnh", "Medical history")}
                </label>
                <textarea
              ...
              <div>
                <label className="mb-1 block text-sm font-bold text-slate-700">
                  {tr("Dị ứng thuốc", "Medication allergies")}
                </label>
                <textarea
            ...
            <button
              ...
              className="mt-6 w-full rounded-full bg-sky-600 py-3 font-bold text-white hover:bg-sky-700 disabled:opacity-50"
            >
              {profileSaving
                ? tr("Đang lưu...", "Saving...")
                : tr("Lưu hồ sơ", "Save profile")}
            </button>
          </div>
        </div>
      )}
      ...
      {showDoctorProfileModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-[2rem] bg-white p-8 shadow-2xl animate-in fade-in zoom-in-95">
            <div className="mb-6 flex items-start justify-between">
              <h3 className="text-2xl font-black tracking-tight text-slate-900">
                {tr("Hồ sơ chuyên môn", "Professional profile")}
              </h3>
              <button
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="mb-1 block text-sm font-bold text-slate-700">
                  {tr("Kinh nghiệm (năm)", "Experience (years)")}
                </label>
                <input
                  type="number"
              ...
              <div>
                <label className="mb-1 block text-sm font-bold text-slate-700">
                  {tr("Tiểu sử & kinh nghiệm", "Biography & experience")}
                </label>
                <textarea
                  ...
                  placeholder={tr(
                    "Giới thiệu bản thân, nơi công tác...",
                    "Introduce your background and workplace...",
                  )}
                />
              </div>
            </div>
            ...
            >
              {doctorProfileSaving
                ? tr("Đang gửi...", "Submitting...")
                : tr(
                    "Lưu và gửi quản trị viên duyệt",
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-[2rem] bg-white p-8 shadow-2xl animate-in fade-in zoom-in-95">
            <div className="mb-6 flex items-start justify-between">
              <div>
                <h3 className="text-2xl font-black tracking-tight text-slate-900">
                  {tr("Thêm nhắc uống thuốc", "Add medication reminder")}
                </h3>
                <p className="text-xs text-slate-500 mt-1">
                  {tr(
                    "Chọn đơn thuốc từ lịch sử khám của bạn",
                    "Select a prescription from your consultation history",
                  )}
                </p>
              </div>
              <button
            ...
            <div className="space-y-4">
              <div>
                <label className="mb-1.5 block text-xs font-bold uppercase tracking-[0.15em] text-slate-500">
                  {tr("Chọn đơn thuốc", "Select prescription")}
                </label>
                {allPrescriptions.length === 0 ? (
                  <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-700 font-medium">
                    ...
                    {tr(
                      ...
                      "You do not have any prescriptions yet. Complete at least one consultation first.",
                    )}
                  </div>
                ) : (
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const selected = allPrescriptions.find(
                        (p) => p.id === pid,
                      );
                      setReminderForm({
                        ...reminderForm,
                        prescriptionId: pid,
                        medicineName: selected
                          ? selected.medicines
                              .split(",")[0]
                              .split("\n")[0]
                              .trim()
                              .substring(0, 60)
                          : reminderForm.medicineName,
                      });
                    }}
                  ...
                  >
                    ...
                    {allPrescriptions.map((p) => (
                      <option key={p.id} value={p.id}>
                        #{p.id} — {p.diagnosis.substring(0, 40)}
              ...
              <div>
                <label className="mb-1.5 block text-xs font-bold uppercase tracking-[0.15em] text-slate-500">
                  {tr("Tên thuốc cần nhắc", "Medication name")}
                </label>
                <input
                  type="text"
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-100"
                  placeholder={tr(
            ...
            <button
              onClick={addReminder}
              disabled={reminderSaving || allPrescriptions.length === 0}
              className="mt-6 w-full rounded-full bg-gradient-to-r from-violet-600 to-purple-500 py-3.5 font-bold text-white shadow-lg shadow-violet-600/25 hover:from-violet-700 hover:to-purple-600 disabled:opacity-50 transition active:scale-[0.98]"
            >
              {reminderSaving
                ? tr("Đang lưu...", "Saving...")
        ...
        <section className="rounded-[2rem] border border-slate-100 bg-white p-6 shadow-[0_24px_80px_rgba(15,23,42,0.06)] lg:col-span-2">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-sky-700">
                {tr("Luồng lịch hẹn", "Appointment flow")}
              </p>
          ...
          <div className="mt-5 space-y-3">
            {data.statusStats.map((item) => {
              const percent =
                data.stats.appointmentCount === 0
                  ? 0
                  : Math.round(
                      (item._count.status / data.stats.appointmentCount) * 100,
                    );
              return (
                <div key={item.status}>
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-sm text-slate-600">
                      {localizeSpecialty(doctor.specialty)}
                    </p>
                  </div>
                  <span
                    className={`rounded-full px-3 py-1 text-xs font-semibold ${doctor.isOnline ? "bg-emerald-100 text-emerald-700" : "bg-slate-200 text-slate-600"}`}
                  >
                    {doctor.isOnline
                      ? tr("Trực tuyến", "Online")
                      : tr("Ngoại tuyến", "Offline")}
                  </span>
                </div>
                <div className="mt-3 flex flex-wrap gap-3 text-sm text-slate-600">
                  ...
                  <span>
                    ...
                    {tr("bệnh nhân", "patients")}
                  </span>
                  <span>
                    {doctor.yearsExp}{" "}
                    {tr("năm kinh nghiệm", "years of experience")}
                  </span>
                </div>
              </div>
            ))}
            ...
            <table className="w-full text-left text-sm text-slate-600">
              <thead className="bg-slate-50 text-xs font-bold uppercase text-slate-500">
                <tr>
                  ...
                  <th className="px-4 py-3">
                    {tr("Kinh nghiệm", "Experience")}
                  </th>
                  <th className="px-4 py-3 text-right">
                    {tr("Hành động", "Actions")}
                  </th>
                </tr>
              </thead>
                  ...
                  <tr>
                    <td colSpan={4} className="py-4 text-center text-slate-400">
                      {tr(
                        ...
                        "No profiles are awaiting approval",
                      )}
                    </td>
                  </tr>
                ) : (
                  pendingDoctors.map((doc) => (
                    <tr key={doc.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3">
                        {doc.experienceYears} {tr("năm", "years")}
                      </td>
                      <td className="px-4 py-3 text-right flex items-center justify-end gap-2">
                        <button
                          onClick={() =>
                            approveDoctorProfile(doc.userId, "APPROVED")
                          }
                          className="rounded-full bg-emerald-100 text-emerald-700 px-3 py-1 text-xs font-bold hover:bg-emerald-200"
                        >
                          {tr("Phê duyệt", "Approve")}
                        </button>
                        <button
                          onClick={() =>
                            approveDoctorProfile(doc.userId, "REJECTED")
                          }
                          className="rounded-full bg-rose-100 text-rose-700 px-3 py-1 text-xs font-bold hover:bg-rose-200"
                        >
                          {tr("Từ chối", "Reject")}
                        </button>
                      </td>
              ...
              <tbody className="divide-y divide-slate-100">
                {data.allUsers?.map((user) => (
                  <tr key={user.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`rounded-full px-2 py-1 text-[10px] font-black tracking-wide ${user.role === "DOCTOR" ? "bg-sky-100 text-sky-700" : user.role === "ADMIN" ? "bg-amber-100 text-amber-700" : "bg-emerald-100 text-emerald-700"}`}
                      >
                        {roleName(user.role)}
                      </span>
                    </td>
                ...
                {invoices.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="py-4 text-center text-slate-400">
                      {tr("Không có hóa đơn nào", "No invoices found")}
                    </td>
                  </tr>
                ) : (
                  invoices.map((inv) => (
                    <tr key={inv.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3">
                        {inv.patient?.fullName ||
                          inv.appointment?.patient?.fullName ||
                          "---"}
                      </td>
              ...
              <tbody className="divide-y divide-slate-100">
                {data.recentAppointments.map((appt) => (
                  <tr key={appt.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      {formatDate(appt.appointmentDate, language)}{" "}
                      {appt.startTime}
                    </td>
                    <td className="px-4 py-3">
                      <span
        ...
        <section className="rounded-[2rem] border border-slate-100 bg-white p-6 shadow-[0_24px_80px_rgba(15,23,42,0.06)] lg:col-span-2">
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-4">
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-full border border-sky-200 bg-sky-50 px-4 py-2 text-sm font-semibold text-sky-700 hover:bg-sky-100"
            >
              <Edit3 className="h-4 w-4" />{" "}
              {tr("Cập nhật hồ sơ y tế", "Update medical profile")}
            </button>
          </div>
          {patientProfile && (
          ...
          <div className="mt-4 space-y-3">
            {reminders.length === 0 ? (
              ...
              reminders.map((r) => (
                <div
                  key={r.id}
                  className={`flex items-center justify-between rounded-2xl border p-4 ${r.isActive ? "border-violet-200 bg-violet-50" : "border-slate-200 bg-slate-50 opacity-60"}`}
                >
                  <div className="flex items-center gap-4">
                    <div
                      className={`grid h-12 w-12 place-items-center rounded-xl ${r.isActive ? "bg-violet-600 text-white" : "bg-slate-200 text-slate-500"}`}
                    >
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className={`px-3 py-1.5 text-xs font-semibold rounded-full ${r.isActive ? "bg-white text-violet-700" : "bg-slate-200 text-slate-700"}`}
                    >
                      {r.isActive
                        ? tr("Đang bật", "Enabled")
                        : tr("Đã tắt", "Disabled")}
                    ...
                    <button
                      ...
                      className="grid h-8 w-8 place-items-center rounded-full text-rose-500 hover:bg-rose-100"
                    >
        ...
        <section className="rounded-[2rem] border border-emerald-100 bg-emerald-50 p-6 shadow-[0_24px_80px_rgba(15,23,42,0.06)] lg:col-span-2">
          ...
          <div className="mt-4">
            ...
            <p className="text-sm text-emerald-600/80 mt-1">
              {tr(
                "Dùng để thanh toán phí khám trực tuyến",
                "Used to pay online consultation fees",
              )}
            </p>
          </div>
        </section>
            ...
            <table className="w-full text-left text-sm text-slate-600">
              ...
              <tbody className="divide-y divide-slate-100">
                {invoices.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="py-4 text-center text-slate-400">
                      {tr("Chưa có giao dịch nào", "No transactions yet")}
                    </td>
                  </tr>
                ) : (
                  invoices.map((inv) => (
                    <tr key={inv.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3">
                        {inv.appointment?.doctor?.fullName || "---"}
                      </td>
                      <td className="px-4 py-3">
                        <span
                          className={`rounded-full px-2 py-1 text-[10px] font-black tracking-wide ${
        ...
        <section className="rounded-[2rem] border border-slate-100 bg-white p-6 shadow-[0_24px_80px_rgba(15,23,42,0.06)]">
          <div className="flex items-center justify-between gap-2">
            ...
            {data.upcomingAppointments.filter((a) => a.status !== "CANCELLED")
              .length > 0 && (
              <span className="rounded-full bg-sky-100 px-2.5 py-0.5 text-xs font-black text-sky-700">
                {
          ...
          <div className="mt-4 space-y-3">
            {data.upcomingAppointments.filter((a) => a.status !== "CANCELLED")
              ...
              <p className="text-center py-6 text-sm text-slate-400">
                {tr("Chưa có lịch hẹn sắp tới", "No upcoming appointments")}
              </p>
            ) : (
                ...
                .map((appointment) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <p className="font-bold text-slate-900">
                          {formatDoctorName(appointment.doctor?.fullName)}
                        </p>
                        <p className="text-sm text-slate-600">
                    ...
                    <div className="mt-3 flex items-center gap-2">
                      ...
                      appointment.status === "ACCEPTED" ? (
                        <button
                          onClick={() =>
                            navigate(
                              `/clinic?doc=${appointment.doctorId ?? 1}&appointmentId=${appointment.id}`,
                            )
                          }
          ...
          <section className="rounded-[2rem] border border-rose-100 bg-rose-50/50 p-6 shadow-[0_24px_80px_rgba(15,23,42,0.04)]">
            <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.2em] text-rose-600">
              ...
              {tr(
                ...
                "Cancelled appointments & refund status",
              )}
            </div>
            <div className="mt-4 space-y-3">
              {data.upcomingAppointments
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <span className="rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-wide bg-rose-100 text-rose-700">
                      ...
                      {relatedInvoice && (
                        ...
                        >
                          {relatedInvoice.status === "REFUNDED" ? (
                            <>
                              <span>✅</span> {tr("Đã hoàn tiền", "Refunded")}{" "}
                              <strong>
                                {relatedInvoice.amount.toLocaleString(
                                  language === "vi" ? "vi-VN" : "en-US",
                                )}
                                đ
                              </strong>{" "}
                              {tr("vào ví của bạn.", "to your wallet.")}
                            </>
                          ) : (
                            <>
                              <span>⏳</span>{" "}
                              {tr("Đang xử lý hoàn tiền", "Processing refund")}{" "}
                              <strong>
                                {relatedInvoice.amount.toLocaleString(
                                  language === "vi" ? "vi-VN" : "en-US",
                                )}
        ...
        <section className="rounded-[2rem] border border-slate-100 bg-white p-6 shadow-[0_24px_80px_rgba(15,23,42,0.06)]">
          ...
          <div className="mt-4 space-y-3">
            {data.completedAppointments.length === 0 ? (
              <p className="text-center py-6 text-sm text-slate-400">
                {tr("Chưa có lịch sử khám bệnh", "No consultation history")}
              </p>
            ) : (
              data.completedAppointments.map((appointment) => (
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <span className="rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-wide bg-slate-200 text-slate-700">
                        ...
                        <button
                          onClick={() =>
                            setSelectedPrescription({
                              ...
                              diagnosis:
                                appointment.prescriptions![0].diagnosis,
                              medicines:
                                appointment.prescriptions![0].medicines,
                              verificationToken:
                                appointment.prescriptions![0].verificationToken,
                              revokedAt:
                                appointment.prescriptions![0].revokedAt,
                              aiSummary:
                                appointment.aiSummaries?.[0]?.aiSummary,
                              suggestedMedicines:
                                appointment.aiSummaries?.[0]
                                  ?.suggestedMedicines,
                            })
                          }
      ...
      <>
        ...
        <section className="rounded-[2rem] border border-slate-100 bg-white p-6 shadow-[0_24px_80px_rgba(15,23,42,0.06)] lg:col-span-2">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              <div>
                ...
                <p className="font-semibold text-sky-600">
                  {data.profile?.specialty
                    ? localizeSpecialty(data.profile.specialty)
                    : tr("Chưa cập nhật chuyên khoa", "Specialty not updated")}
                </p>
                <p className="text-sm text-slate-500">
            ...
            <div className="text-right">
              ...
              <div>
                <button
                  onClick={() => setShowDoctorProfileModal(true)}
                  className="flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100 transition"
                >
                  <Edit3 className="h-4 w-4" />{" "}
                  {tr("Cập nhật hồ sơ", "Update profile")}
                </button>
              </div>
            </div>
          </div>
          {data.profile?.bio && (
            <div className="mt-6 rounded-2xl bg-slate-50 p-4">
              ...
              <p className="mt-1 text-sm font-medium text-slate-700 leading-relaxed whitespace-pre-wrap">
                {data.profile.bio}
              </p>
            </div>
          )}
        ...
        <section className="rounded-[2rem] border border-slate-100 bg-white p-6 shadow-[0_24px_80px_rgba(15,23,42,0.06)] lg:col-span-2">
          ...
          <div className="mb-4">
            ...
            <p className="mt-2 text-xs text-slate-500">
              {tr(
                "Hệ thống tự mở ca từ 09:00 đến 18:00, nghỉ trưa 12:00–13:00. Ca màu đỏ đã có bệnh nhân đặt.",
                "Slots open automatically from 09:00 to 18:00, with a lunch break from 12:00 to 13:00. Red slots are booked.",
              )}
            </p>
            {loadingSchedule && (
              <p className="mt-2 text-xs font-semibold text-sky-600">
                {tr("Đang tải lịch...", "Loading schedule...")}
              </p>
            )}
          </div>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
            {TIME_SLOTS.map((slot) => {
              ...
              const scheduleItem = doctorSchedules.find(
                (s) => s.startTime === start,
              );
              ...
              return (
                ...
                >
                  {slot}
                  {isBooked && (
                    <span className="block text-[9px] mt-1 text-rose-500 font-black">
                      {tr("Đã đặt", "Booked")}
                    </span>
                  )}
                </button>
        ...
        <section className="rounded-[2rem] border border-slate-100 bg-white p-6 shadow-[0_24px_80px_rgba(15,23,42,0.06)] lg:col-span-2">
          ...
          <div className="mt-4 space-y-2">
            <h2 className="text-2xl font-black tracking-tight">
              {formatDoctorName(data.doctor.fullName)}
            </h2>
            ...
            <div className="mt-3 inline-flex items-center gap-2 rounded-xl bg-emerald-50 px-3 py-1.5 text-sm font-bold text-emerald-700 border border-emerald-100">
              ...
              {(data.stats.completedAppointments * 100000).toLocaleString(
                language === "vi" ? "vi-VN" : "en-US",
              )}{" "}
              VNĐ
            </div>
        ...
        <section className="rounded-[2rem] border border-slate-100 bg-white p-6 shadow-[0_24px_80px_rgba(15,23,42,0.06)] lg:col-span-2">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.2em] text-sky-700">
              <CalendarDays className="h-4 w-4" />
              {tr("Lịch hẹn từ bệnh nhân", "Patient appointments")} (
          ...
          <div className="mt-4 space-y-3">
            {appointmentsList.length === 0 ? (
              <p className="text-center py-6 text-sm text-slate-400">
                {tr(
                  "Chưa có lịch hẹn nào từ bệnh nhân",
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    ...
                    {(appointment.status === "CONFIRMED" ||
                      ...
                      <button
                        onClick={() =>
                          navigate(
                            `/clinic?doc=${authUser?.id ?? 1}&appointmentId=${appointment.id}`,
                          )
                        }
            ...
            {data.completedAppointments.filter((a) => a.review).length === 0 ? (
              ...
              data.completedAppointments
                ...
                .map((appointment) => (
                  <div
                    key={appointment.id}
                    className="rounded-3xl border border-amber-100 bg-white p-4"
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <p className="font-bold text-slate-900">
                ...
                <button
                  onClick={() =>
                    setSelectedPrescription({
                      ...
                      appointmentId: 0,
                      date: formatDate(
                        item.appointment?.appointmentDate ?? item.createdAt,
                        language,
                      ),
                      doctorName: formatDoctorName(
                  ...
                  className="mt-3 inline-flex items-center gap-2 rounded-full bg-sky-600 px-4 py-2 text-xs font-bold text-white transition hover:bg-sky-700"
                >
                  <ShieldCheck className="h-3.5 w-3.5" />
                  {tr("Xem QR xác minh", "View verification QR")}
                </button>
              </div>
            ))}
          </div>
